...
ENV MESON_OPTIONS="-Dvaapi=enabled -Dgpl=enabled -Dgst-examples=disabled -Dexamples=disabled -Dtests=disabled -Ddoc=disabled -Dqt5=disabled -Dpython=disabled -Dges=disabled -Ddevtools=disabled -Dintrospection=disabled -Dlibnice:gupnp=disabled -Dgstreamer-vaapi:x11=disabled -Dgstreamer-vaapi:encoders=enabled"

# Build and install GStreamer
RUN cd gstreamer && \
  meson setup build --prefix=/usr ${MASON_OPTIONS} && \
  ninja -C build && \
...
# Check versions
RUN ruby -v && gst-launch-1.0 --version
